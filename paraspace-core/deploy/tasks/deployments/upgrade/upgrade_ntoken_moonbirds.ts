...
import {deployGenericMoonbirdNTokenImpl} from "../../../helpers/contracts-deployments";
import {
...
export const upgradeNTokenMoonBirds = async () => {
  ...
  const nTokenImplementation = await deployGenericMoonbirdNTokenImpl(
    poolAddress,
  ...
  for (let i = 0; i < allTokens.length; i++) {
    ...
    if ((await nToken.getXTokenType()) == XTokenType.NTokenMoonBirds) {
  ...
  console.log("upgrade all NTokenMoonBirds implementation finished.");
};
